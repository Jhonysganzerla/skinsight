...
- **Expansão do banco (pussy/solitude/etc. — PENDENTE DE FONTE):** o pipeline está pronto (schema do `rare_patterns.json` + `sweep-patterns.mjs` + invariantes em `pattern-bank.test.ts` no CI); adicionar família = adicionar entrada no JSON com seeds + fonte. **Não inventar seeds.** Próximos passos: (a) Jhony fornece a lista de famílias desejadas com fonte (nomes que a comunidade usa: pussy pattern, solitude, doggo/Heat, Kraken art-position etc.); (b) investigar se a resposta da CS.Money expõe metadados de rare pattern por item (rodar um scan com `skinsight:debug` e olhar os raw keys — se houver `overpay.pattern`/`rarePattern`, dá pra **validar só o que tem overpay** direto da fonte que paga o overpay).
